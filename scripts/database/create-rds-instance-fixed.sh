...
# WordPress RDS Instance Creation Script (Fixed Version)
...
read -p "Enter DB Subnet Group Name (optional, press Enter to skip): " DB_SUBNET_GROUP
...
if [ $? -eq 0 ]; then
    ...
    echo "2. Check status: aws rds describe-db-instances --db-instance-identifier $RDS_IDENTIFIER --query 'DBInstances[0].DBInstanceStatus' --output text"
